...
const PdfCreationPanel: React.FC = () => {
  const { status, activeRunId, resumeFromStage, refreshStatus } = usePipeline();
...
const handleDownload = useCallback(
  ...
  const handleCreatePdf = useCallback(async () => {
    if (!activeRunId || !hasReadyMappings || isQueuing) return;
    setIsQueuing(true);
    setQueueError(null);
  ...
  }, [activeRunId, resumeFromStage, hasReadyMappings, refreshStatus, isQueuing]);
