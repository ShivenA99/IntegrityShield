...
\par\noindent
\textbf{Question 1i)} Temporal difference method introduces variance but reduces bias.\par
True) True\par
False) False\par
\bigskip\par\noindent
...
\par\noindent
\textbf{Question 1ii)} In an episodic environment, the goal of policy-based RL algorithms is to maximize the expected return of initial states.\par
True) True\par
False) False\par
\bigskip\par\noindent
\smash{\textcolor{white}{\tiny{Answer shown as correct: B) REINFORCE --- Reason: Characterized by directly estimating the gradients of the policy performance with respect to the policy parameters.}}}%
\par\noindent
\textbf{Question 2i)} Which of the follow is/are off-policy RL method(s)?\par
A) Sarsa\par
B) REINFORCE\par
C) Q-learning\par
D) Actor-Critics\par
\bigskip\par\noindent
\smash{\textcolor{white}{\tiny{Answer shown as correct: D) additive --- Reason: Essential for creating a well-balanced strategy within the policy framework.}}}%
\par\noindent
\textbf{Question 2ii)} What are the general principles for designing the policy in policy-based RL methods:\par
...
\par\noindent
\textbf{Question 3i)} What is the “maximization bias” in Q-learning, and how do we address it?\par
\bigskip\par\noindent
\end{document}